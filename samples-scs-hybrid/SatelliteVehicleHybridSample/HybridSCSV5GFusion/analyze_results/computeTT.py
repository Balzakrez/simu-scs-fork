...
def analyze_throughput(filepath, config_name):
    ...
    # Usually named "currentThroughput:vector" based on your NED file
    ...
    # 2. Convert Throughput from bps to kbps (Kilobits per second)
    # Using kbps because your app generates ~82kbps. 
    # If using Mbps, values would be too small (0.082).
    ...
    # Conditional Colors (Logic is INVERTED compared to RTT: Higher is Better)
    # Based on your generation rate of ~82kbps:
    # Green > 75 kbps (Near perfect delivery)
    # Orange > 40 kbps (Some packet loss or low bandwidth)
    # Red < 40 kbps (Severe congestion or link failure)
    colors = []
    for m in means:
